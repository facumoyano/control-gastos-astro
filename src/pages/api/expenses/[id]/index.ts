...
const expensesRef = db.collection("incomes");
...
export const POST: APIRoute = async ({ params, redirect, request }) => {
  ...
  try {
    const doc = await expensesRef.doc(params.id).get();
    ...
    await expensesRef.doc(params.id).update({
      title,
      amount,
      date,
      category,
...
export const DELETE: APIRoute = async ({ params, redirect, request }) => {
  ...
  try {
    ...
    await expensesRef.doc(params.id).delete();
  } catch (error) {
    return new Response("Algo salió mal", {
      status: 500,
    });
